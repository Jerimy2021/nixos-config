...
defaultwallpaper="$HOME/Pictures/Wallpapers/tokyoGoulRe.png"
...
if [ -f $wallpapereffect ]; then
    ...
    echo ":: Force setting wallpaper via swww"
    swww img "$wallpaper" --transition-type grow --transition-pos 0.5,0.5 --transition-step 90 --transition-fps 60
    # ----------------------------------------------------
fi
else
    effect="off"
...
# Execute matugen
# -----------------------------------------------------
# Si la detección falló (está vacío), forzamos modo oscuro (1)
if [ -z "$THEME_PREF" ]; then
    THEME_PREF=1
fi

# Ejecutar matugen usando el comando global (sin ruta .local)
